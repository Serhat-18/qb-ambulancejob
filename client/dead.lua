...
Citizen.CreateThread(function()
    ...
    if PlayerData.metadata['inlaststand'] or PlayerData.metadata['isdead'] then
        TriggerEvent('qbcore:playerdead',true)
        TriggerServerEvent('hospital:server:RespawnAtHospital')
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Ölü olarak giriş yaptığın için üzerin temizlendi ve hastaneye gönderildin. :)', 'info')
        TriggerServerEvent('hospital:server:SetDeathStatus', false)
        TriggerEvent('qbcore:playerdead', false)
    end
end)
