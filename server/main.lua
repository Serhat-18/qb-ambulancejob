...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("cash", Config.BillCost , "respawned-at-hospital")
end)
		...
		for k, v in pairs(Config.Locations["jailbeds"]) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('QBCore:Notify', src, "Tüm eşyaların alındı!", 'error', 1500)
				end
				Player.Functions.RemoveMoney("cash", Config.BillCost, "respawned-at-hospital")
				return
			end
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('QBCore:Notify', src, "Yeterli paran yok!", 'error', 1500)
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        for _, v in pairs(players) do
            if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
				TriggerClientEvent('QBCore:Notify', v.PlayerData.source, "Hastanede bir doktora ihtiyaç var!", 'error', 1500)
            end
        end
		...
		TriggerClientEvent('QBCore:Notify', src, "Doktora gerekli bilgi gönderildi.", 'success', 1500)
    end
end)
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('QBCore:Notify', helperId, "Bu kişiye yardım edemezsin!", 'error', 1500)
	end
end)
...
RegisterNetEvent('hospital:server:removeIfaks', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	Player.Functions.RemoveItem('antizin', 1)
end)
...
QBCore.Commands.Add("status", Lang:t('info.check_health'), {}, false, function(source, _)
	...
	if Player.PlayerData.job.name == "ambulance" then
		...
		TriggerClientEvent('QBCore:Notify', src, "Doktor değilsin veya mesai dışındasın!", 'error', 1500)
	end
end)
...
QBCore.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	...
	if args[1] then
		...
		if Player then
			...
			TriggerClientEvent('QBCore:Notify', src, "Oyuncu çevrimiçi değil!", 'error', 1500)
		end
	else
...
QBCore.Functions.CreateUseableItem("antizin", function(source, item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
